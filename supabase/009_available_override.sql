-- =============================================================
-- Manual availability override: a supervisor can pull a dispatcher out of the
-- live pool (e.g. they disconnected) regardless of the schedule.
-- Run: node --env-file=.env.local scripts/migrate.mjs 009_available_override.sql
-- =============================================================

alter table dispatchers add column if not exists available_override text; -- null | 'offline'

-- Rebuild refresh_pool_activo to exclude manually-offline dispatchers.
...
begin
  create temporary table _active on commit drop as
    ...
    where d.status = 'activo'
      and d.available_override is distinct from 'offline'   -- respect manual offline
